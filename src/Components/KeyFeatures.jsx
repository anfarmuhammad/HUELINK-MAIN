const KeyFeatures = () => {
  ...
  return (
    <section className="py-16 bg-black from-white to-slate-100 dark:from-[#0f172a] dark:to-[#1e293b]">
      <div className="container mx-auto px-4">
        <h2 className="text-3xl font-bold text-center text-white dark:text-white mb-12">Why Choose Us?</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <div
              key={index}
              className="bg-black text-white h-[300px] backdrop-blur-md border border-white/20 p-6 rounded-2xl shadow-lg text-center transition duration-300 hover:scale-105"
            >
              <div className="flex justify-center mb-4">{feature.icon}</div>
              <h3 className="text-xl font-semibold text-white dark:text-white mb-2">{feature.title}</h3>
              <p className="text-white dark:text-gray-300">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
